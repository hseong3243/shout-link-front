...
export default {
  ...
  methods: {
    createHubLinkApiCall() {
      api.post(`/api/hubs/${this.hubId}/links`, {
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form>
            ...
            <v-text-field
                v-model="createHubLinkRequest.url"
                label="링크 url"
                density="comfortable"
                variant="solo">
            </v-text-field>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn @click="createHubLinkApiCall">추가</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
